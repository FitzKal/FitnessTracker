...
import {deleteMonthlyGoal} from "../../../services/GoalService.ts";
import {toast} from "react-toastify";

export default function DeleteMonthlyGoalForm(prop:{goalDetails:MonthlyGoal, isDeleting:boolean, deleteHandler:() => void}){
        ...
        `}
            >
                ...
                <p className="text-lg sm:text-2xl mb-4 sm:mb-6">
                    You are about to delete the goal from {prop.goalDetails.startDate.toString()} to {prop.goalDetails.finishDate.toString()} <strong className="font-bold text-red-500">permanently</strong>?
                </p>
                ...
                <div className="flex flex-col sm:flex-row justify-center sm:justify-between gap-4 sm:gap-10">
                    <button
                        type="button"
                        className="px-6 py-2 rounded-xl bg-gray-200 hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-700 font-semibold transition-colors w-full sm:w-auto"
                        onClick={prop.deleteHandler}
                    >
                        Cancel
                    </button>
                    <button
                        ...
                        onClick={() => {
                            deleteGoalMutation.mutate(prop.goalDetails.monthlyGoalId)
                        }}>
