...
import {deleteDailyGoal} from "../../../services/DailyGoalService.ts";

export default function DeleteDailyGoalForm(prop:{goalDetails:dailyGoal, isDeleting:boolean, deleteHandler:() => void}){
    const queryClient = useQueryClient();
    const deleteGoalMutation = useMutation({
        mutationFn:(id:number)=> deleteDailyGoal(id),
        onSuccess:()=>{
            queryClient.invalidateQueries({queryKey:["getAllDailyGoals"],exact: false});
            queryClient.invalidateQueries({queryKey:["dailyGoalById"],exact: false});
        ...
        <>
            ...
            >
                ...
                <p className="text-lg sm:text-2xl mb-4 sm:mb-6">
                    You are about to delete the goal from {prop.goalDetails.dateOfExercise.toString()} <strong className="font-bold text-red-500">permanently</strong>?
                </p>
                <p className="text-base sm:text-xl mb-6 sm:mb-10">
                    Deleting this goal will result in the complete removal of <strong className="font-bold text-red-500">all of its exercise data</strong>!
                </p>
